<template>
	<main class="website-container">
		<section v-if="settlement" class="description">
		...
		<section v-if="settlement.players" class="stats">
			...
			<div v-if="lastSeason > 0" class="seasons">
				...
				<table v-if="seasonData" class="table-panel">
					<thead>
						<tr>
							<th>LP</th>
							<th>Gracz</th>
					...
					<tbody>
						<template v-for="(record, index) in seasonData">
							<tr v-for="(line, jIndex) in record" :key="index + jIndex">
								<th>{{ index + jIndex + 1 }}</th>
								<th>{{ line.player }}</th>
								<th>{{ line.name }}</th>
								<th>{{ line.date }}</th>
								<th>{{ line.points }} pkt</th>
							</tr>
						</template>
					</tbody>
				</table>
		...
		computed: {
			...
			{				
				return this.settlement.players.map(p => {
					return p.scores.filter(s => s.event.season == this.season)
						.map(e => {		
							return{
			...
			lastSeason()
			{
				return Math.max(...this.settlement.players.map(p => {
					return Math.max(...p.scores.map(e => {
...
<style lang="scss" src="./SettlementDetails.scss" scoped></style>